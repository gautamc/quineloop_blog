...
"ip2locations":http://www.ip2location.com/ is a service that we can use to get location information of a ip address. Essentially, ip2locations provides to a collection of data files to choose from and download that we query via a "C library":http://www.ip2location.com/c.aspx that they provide. There is a ruby library that provide a ruby interface to the this C library.
...
STR2CSTR is a ruby 1.8 thing. With ruby 1.9, we should be using StringValuePtr. It takes a VALUE and return a C string. Before we pass the value to StringValuePtr, we must ensure that its mutable - hence the calls to rb_str_dup and rb_str_modify.
